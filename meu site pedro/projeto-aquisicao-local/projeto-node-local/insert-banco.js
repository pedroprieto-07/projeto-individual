...
function registrar_leitura(ocupacao) {
    ...
    banco.conectar().then(() => {
        ...
        return banco.sql.query(`
            INSERT INTO Historico (ocupacao, data_hora, fksensor) 
            SELECT top 1    ${ocupacao}, CONVERT(Datetime, '${agora()}', 120), 1
            FROM Historico 
            WHERE 
            NOT(${ocupacao} IN (
                SELECT top 1 ocupacao FROM Historico 
                where fksensor = 1
        ...
        // delete from Historico where id_historico not in 
        // (select top ${registros_mantidos_tabela_leitura} id_historico from Historico order by id_historico desc);`)
        .then(() => {
